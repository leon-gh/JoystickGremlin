...
class LaunchAppWidget(gremlin.ui.input_item.AbstractActionWidget):

    """Widget for loading the profile."""

    def __init__(self, action_data, parent=None):
        super().__init__(action_data, parent=parent)
        assert isinstance(action_data, LaunchApp)
    ...
    def _populate_ui(self):
        self.file_path.setText(self.action_data.application_file)
    ...
    def _new_executable(self):
        ...
        if fname != "":
            self.action_data.application_file = fname
            self._populate_ui()
...
class LaunchAppFunctor(AbstractFunctor):
    ...
    def __init__(self, action):
        ...
        self.application_file = action.application_file

    def process_event(self, event, value):
        gremlin.gremlin_ui._do_launch_app(self.application_file)
        return True


class LaunchApp(AbstractAction):
    ...
    def _parse_xml(self, node):
        self.application_file = node.get("file")

    def _generate_xml(self):
        node = ElementTree.Element("launch-app")
        node.set("file", self.application_file)
        return node

    def _is_valid(self):
        return self.application_file is not None and len(self.application_file) > 0
